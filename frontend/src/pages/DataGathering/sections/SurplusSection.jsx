...
export default function SurplusSection({ family, isReadOnly }) {
  ...
  const exportToExcel = () => {
    ...
    dataSheetData.push(['Member', 'Income Type', 'Description', 'Monthly Amount', 'Annual Amount', 'Growth %', 'Up to Year', 'Market Value']);
    incomeDetails.forEach(inc => {
      const details = inc.details || {};
      const memberNames = (inc.member_ids || []).map(mid => {
        const m = members.find(mem => mem.id === mid || String(mem.id) === String(mid));
        return m?.name || '';
      }).filter(n => n).join(', ') || 'N/A';
      ...
      switch (inc.category) {
        ...
        case 'business':
          ...
          monthlyAmt = parseFloat(details.net_income_monthly) || annualAmt / 12;
          growthRate = details.avg_growth_rate || '';
          description = details.business_name || details.business_type || '';
          break;
        case 'rental':
          monthlyAmt = parseFloat(details.rent_per_month) || 0;
          annualAmt = parseFloat(details.annual_rent) || monthlyAmt * 12;
          description = details.property_details || '';
          marketValue = parseFloat(details.market_value) || '';
          break;
        case 'epf':
          ...
          marketValue = parseFloat(details.market_value) || parseFloat(details.current_balance) || '';
          break;
        case 'ppf':
          monthlyAmt = parseFloat(details.monthly_contribution) || 0;
          annualAmt = parseFloat(details.annual_contribution) || monthlyAmt * 12;
        ...
        case 'shares_pms':
          monthlyAmt = parseFloat(details.monthly_contribution) || 0;
          annualAmt = parseFloat(details.annual_contribution) || monthlyAmt * 12;
          description = 'Shares/PMS Investment';
          marketValue = parseFloat(details.market_value) || parseFloat(details.current_value) || '';
          break;
        case 'mutual_fund':
          monthlyAmt = parseFloat(details.sip_amount) || 0;
          annualAmt = parseFloat(details.annual_amount) || monthlyAmt * 12;
          description = details.fund_name || 'Mutual Fund SIP';
          marketValue = parseFloat(details.market_value) || parseFloat(details.current_value) || '';
          break;
        case 'pension':
          annualAmt = parseFloat(details.amount_yearly) || parseFloat(details.amount) * 12 || 0;
          monthlyAmt = annualAmt / 12;
          description = details.pension_type || 'Pension';
          break;
        case 'cash':
          marketValue = parseFloat(details.bank_balance) || 0;
          description = details.bank_name || 'Cash/Bank Balance';
          break;
        case 'fd':
          annualAmt = parseFloat(details.interest_amount) || 0;
          description = details.bank_name || 'Fixed Deposit';
          marketValue = parseFloat(details.investment_value) || '';
          break;
        case 'bonds':
          annualAmt = parseFloat(details.coupon_amount) || 0;
          description = details.bond_name || 'Bonds';
          marketValue = parseFloat(details.investment_value) || parseFloat(details.market_value) || '';
          break;
        case 'nps':
          monthlyAmt = parseFloat(details.monthly_contribution) || 0;
          annualAmt = parseFloat(details.annual_contribution) || monthlyAmt * 12;
          description = 'National Pension Scheme';
          marketValue = parseFloat(details.current_value) || '';
          break;
        case 'commodities':
          description = details.commodity_type || 'Gold/Commodities';
          marketValue = parseFloat(details.market_value) || '';
          break;
        case 'insurance_income':
          annualAmt = parseFloat(details.premium_yearly) || 0;
          description = details.policy_name || 'Insurance';
          marketValue = parseFloat(details.sum_assured) || '';
          break;
        default:
          annualAmt = parseFloat(details.annual_amount) || parseFloat(details.amount) || 0;
          monthlyAmt = annualAmt / 12;
          description = inc.category || 'Other';
      }
      
      const categoryLabel = (inc.category || 'other').replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase());
      dataSheetData.push([
        memberNames, 
        categoryLabel, 
        description,
        Math.round(monthlyAmt), 
        Math.round(annualAmt), 
