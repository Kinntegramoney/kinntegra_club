                            ...
                            <div className="min-w-0">
                              <HoverCard>
                                <HoverCardTrigger asChild>
                                  <span className="font-medium block truncate cursor-pointer hover:text-blue-600 hover:underline flex items-center gap-1">
                                    {partner.name}
                                ...
                                <HoverCardContent className="w-80" align="start">
                                  <div className="space-y-3">
                                    ...
                                    <div className="bg-gray-50 rounded-lg p-3 space-y-2 text-sm">
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-1">
                                          <span className="font-mono font-medium">{partner.pan}</span>
                                          <button 
                                            onClick={(e) => { e.stopPropagation(); copyToClipboard(partner.pan, `pan-${partner.id}`); }}
                                            className="p-1 hover:bg-gray-200 rounded"
                                          >
                                            {copiedField === `pan-${partner.id}` ? <Check className="h-3 w-3 text-green-500" /> : <Copy className="h-3 w-3 text-gray-400" />}
                                          </button>
                                        </div>
                                      </div>
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-1">
                                          <span className="font-mono font-medium">{partner.initial_password || '••••••••'}</span>
                                          {partner.initial_password && (
                                            <button 
                                              onClick={(e) => { e.stopPropagation(); copyToClipboard(partner.initial_password, `pwd-${partner.id}`); }}
                                              className="p-1 hover:bg-gray-200 rounded"
                                            >
                                              {copiedField === `pwd-${partner.id}` ? <Check className="h-3 w-3 text-green-500" /> : <Copy className="h-3 w-3 text-gray-400" />}
                                            </button>
                                          )}
                                        </div>
                                      </div>
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        <div className="flex items-center gap-1">
                                          <span className="font-mono font-medium">{partner.initial_pin || '••••'}</span>
                                          {partner.initial_pin && (
                                            <button 
                                              onClick={(e) => { e.stopPropagation(); copyToClipboard(partner.initial_pin, `pin-${partner.id}`); }}
                                              className="p-1 hover:bg-gray-200 rounded"
                                            >
                                              {copiedField === `pin-${partner.id}` ? <Check className="h-3 w-3 text-green-500" /> : <Copy className="h-3 w-3 text-gray-400" />}
                                            </button>
                                          )}
                                        </div>
                                      </div>
                                    ...
                                    {/* Copy All Button */}
                                    {partner.initial_password && partner.initial_pin && (
                                      <Button
                                        variant="outline"
                                        size="sm"
                                        className="w-full"
                                        onClick={(e) => { 
                                          e.stopPropagation(); 
                                          copyToClipboard(
                                            `Login Credentials for ${partner.name}\n\nPAN: ${partner.pan}\nPassword: ${partner.initial_password}\nPIN: ${partner.initial_pin}\n\nLogin: https://kinntegraa.club/login`,
                                            `all-${partner.id}`
                                          ); 
                                        }}
                                      >
                                        {copiedField === `all-${partner.id}` ? <Check className="h-3 w-3 mr-1 text-green-500" /> : <Copy className="h-3 w-3 mr-1" />}
                                    ...
                                    {!partner.initial_password && (
                                      <p className="text-xs text-amber-600 bg-amber-50 p-2 rounded">
                                        Credentials not available. This partner was created before credential storage was enabled.
                                      </p>
                                    )}
                                  </div>
                                </HoverCardContent>
