...
async def download_historical_trades_template(current_user: dict = Depends(get_current_user)):
    ...
    instructions = [
        ...
        "",
        "═══════════════════════════════════════════════════════════════════",
        "IRREGULAR PAYMENTS / PREPAYMENTS",
        "═══════════════════════════════════════════════════════════════════",
        "When you upload a repayment with Principal on a date that doesn't",
        "have a scheduled payment, the system will:",
        "1. Detect it as an IRREGULAR/PREPAYMENT",
...
def calculate_actual_xirr(investment_date: str, investment_amount: float, cashflows: List[dict], actual_repayments: List[dict] = None, bond_start_date: str = None) -> Optional[float]:
    ...
    try:
        ...
        logger.error(f"Actual XIRR calculation error: {e}")
        return None
