...
async def bulk_upload_bonds(
    ...
    if not df_cashflows.empty:
        print(f"DEBUG: Processing cashflows sheet with {len(df_cashflows)} rows")
        print(f"DEBUG: Columns: {df_cashflows.columns.tolist()}")
        for _, crow in df_cashflows.iterrows():
            bc = str(crow.get('bond_code', '')).strip()
            print(f"DEBUG: Processing row for bond_code: '{bc}'")
            if bc and not pd.isna(bc):
                ...
                print(f"DEBUG: date={payment_date}, interest={interest}, principal={principal}")
                if not pd.isna(payment_date):
                    try:
                        ...
                        print(f"DEBUG: Added cashflow for {bc}: {date_str}")
                    except Exception as e:
                        print(f"DEBUG: Exception parsing cashflow: {e}")
        # Sort each bond's cashflows by date
        for bc in cashflows_per_unit_map:
            cashflows_per_unit_map[bc].sort(key=lambda x: x['date'])
        print(f"DEBUG: Final cashflows_per_unit_map keys: {list(cashflows_per_unit_map.keys())}")
    else:
        print("DEBUG: df_cashflows is empty")
